...
from config.apps.catalog.serializers.front import HeaderDataSerializer, BrandSerializer, CategorySerializer
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if cached_brands and cached_categories:
                ...
                }, status=status.HTTP_200_OK, message=ResponseMessage.SUCCESS.value)

            # If not in cache, perform the queries
            categories = (Category.objects.filter(tn_level=1)
                          .only(
                'id',
                'title_ir',
                'title_en',
            ...
            brands = Brand.objects.only("title_ir", "title_en", "image").select_related('image').all()
            ...
            # Set data in cache
            self._set_cached_data('cached_brands', response_data['brands'], timeout=None)  # No expiration for brands
            self._set_cached_data('cached_categories', response_data['categories'],
                                  timeout=None)  # No expiration for categories

            return BaseResponse(response_data, status=status.HTTP_200_OK, message=ResponseMessage.SUCCESS.value)
